...
const TOC = [
  ...
  { id: "experience", label: "停滞期かどうか、当時の自分には判定できなかった" },
  { id: "faq", label: "よくある質問" },
  ...
  return (
    ...
    >
      ...
      <div className="space-y-4">
        ...
        <p>
          ダイエットを続けていると、ほぼ必ず訪れるのが<strong>停滞期</strong>です。ここで多くの人が「もう痩せないんだ」と挫折します。
        </p>
        <p>
      ...
      <section>
        <h2 id="experience" className="text-xl font-bold text-green-700 mb-4 pb-2 border-b border-green-100">
          停滞期かどうか、当時の自分には判定できなかった
        </h2>
        <div className="space-y-4">
          <p>
            この記事の前半で、停滞期かどうかを見分けるチェックリストを出しました。正直に書くと、92kgから落としていた当時の自分は、
            <strong>あの3つを1つも確認できませんでした</strong>。体重も食事も、ほとんど記録していなかったからです。
          </p>
          <p>
            なので「何週間止まった」と具体的に書くことができません。残っているのは、
            <strong>最初の1〜3ヶ月は食事だけで3〜5kg落ちた</strong>
            という大まかな記憶だけで、その先の週ごとの推移は数字として存在しません。ここに具体的な日数や体重を書けば読み物としては締まりますが、それは記録ではなく創作になります。
          </p>
          <div className="bg-red-50 rounded-xl p-4 border border-red-100 space-y-2">
            <p className="text-sm font-bold text-red-700">記録がないと、こうなります</p>
            <p className="text-sm text-gray-700">
              止まったかどうかが<strong>分からない</strong>。「止まった気がする」で判断することになる
            </p>
            <p className="text-sm text-gray-700">
              原因が<strong>特定できない</strong>。摂取が増えたのか、消費が減ったのか、ただの誤差なのかを区別できない
            </p>
            <p className="text-sm text-gray-700">
              対処が<strong>当てずっぽうになる</strong>。とりあえずもっと減らす、という一番危ない選択に向かいやすい
            </p>
          </div>
          <p>
            もうひとつ、今なら分かることがあります。92kgに対する3〜5kgは
            <strong>約3〜5%</strong>
            です。この程度だと体重計の数字は動いていても見た目はほとんど変わりません。記録がなければ、この時期を「止まっている」と受け取ってもおかしくない。実際に止まっていたのか、動いていたのに気づけなかっただけなのかは、
            <strong>今となっては確かめようがありません</strong>
            （この時期の話は
            <a
          ...
          <p>
            だから、自分の経験から一番実用的なことを書くとしたら、停滞期の抜け方ではありません。
            <strong>停滞期かどうかを判定できる記録を、止まる前から持っておくこと</strong>
            です。毎日でなくてかまいません。同じ条件（起床後・トイレのあと）で測って残しておくだけで、次に止まったときに「本当に止まったのか」を確かめられます。
          </p>
          <p>
            この記事の前半に書いた対処法は、どれも
            <strong>止まったことを確認できている人のためのもの</strong>
            です。当時の自分はその手前で、使える状態にありませんでした。
          </p>
        </div>
      </section>
